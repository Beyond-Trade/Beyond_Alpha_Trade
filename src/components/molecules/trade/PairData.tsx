...
function PairData(props:IProps) {
  return (
    <table width="100%">
      ...
      {props.data.pairs.filter((item)=>item.coin.toUpperCase().includes(props.search)||props.search==="").map((item) => (
        <tr onClick={()=>props.onSelect(props.data.marketCoin, item.coin)} className="bg-customGray-100 hover:bg-gray-300 cursor-pointer text-xs text-left text-gray-600 font-medium">
