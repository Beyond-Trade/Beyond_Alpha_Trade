...
function AllTradesTable(props: IProps) {
  return (
    <React.Fragment>
      <table width="100%">
        ...
        {props.data.map((item) => (
          <tr className="text-xs text-left text-gray-600 font-medium hover:bg-gray-300">
            ...
            <td className="py-3 px-3">
              <a href={item.infoURL} target="_blank" className="focus:outline-none text-customBlue-200 underline px-2 py-1 font-bold text-xs rounded-sm">
                VIEW
              </a>
            </td>
          </tr>
      ...
      {props.data.length===0&&<div className="h-48 text-blue-500 text-xs flex justify-center items-center">No data found</div>}
    </React.Fragment>
  );
}
