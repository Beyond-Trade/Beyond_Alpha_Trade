...
function StatesRow() {
  ...
  let totalUSDbBalancePercentage =
    ((USDbBalance.cryptoBalance + USDbBalance.rate) / totalSynthValue) * 100;

  let othersBalPercentage =
    ((totalUSDbBalancePercentage + totalBynPercentage) / totalSynthValue) * 100;
    return (
        ...
        <div className="bg-customGray-100 rounded mr-8 w-full mt-8">
            ...
            <h3 className="font-medium my-6 mx-2"> ${totalSynthValue ? totalSynthValue.toFixed(2) : "0.00"} USD</h3>
        </div>
        <div className="bg-customGray-100 rounded mr-8 w-full mt-8">
            <div className="rounded-t bg-gray-300 text-gray-600 text-xs px-2 py-1 font-medium">
            ...
            <div className="flex">
                ...
                <div>
                    <div className="flex text-xxs text-gray-500 font-medium mt-1">
                        ...
                        <h6>{totalBynPercentage ? totalBynPercentage.toFixed(2) : "00.00"}%</h6>
                    </div>
                    <div className="flex text-xxs text-gray-500 font-medium mt-1">
                        <img src="assets/Icons/Purple.Ellipse.svg" className="h-3" />
                        <h6 className="ml-1 mr-2">USDb</h6>
                        <h6>{totalUSDbBalancePercentage ? totalUSDbBalancePercentage.toFixed(2) : "00.00"}%</h6>
                    </div>
                    <div className="flex text-xxs text-gray-500 font-medium mt-1">
                        <img src="assets/Icons/Purple.Ellipse.svg" className="h-3" />
                        <h6 className="ml-1 mr-2">Others</h6>
                        <h6>{othersBalPercentage ? othersBalPercentage.toFixed(2) : "00.00"}%</h6>
                    </div>
                </div>
            </div>
        </div>
